...
static inline struct casdb_sha1_hash_t
hash_chunk(struct casdb_sha1_hash_t h, const void *mem)
{
    uint32_t buf[80];
    int i;
    uint32_t a;
...
static struct casdb_sha1_hash_t
casdb_sha1_set_initial_hash_values(void)
{
    struct casdb_sha1_hash_t rv;
...
static void
casdb_sha1_reset_context_buf(struct casdb_sha1_hash_context_t *context)
{
...
void
casdb_sha1_hash_init(struct casdb_sha1_hash_context_t *context)
{
    casdb_sha1_reset_context_buf(context);
    context->hash = casdb_sha1_set_initial_hash_values();
}


static size_t
casdb_sha1_handle_leftover(struct casdb_sha1_hash_context_t *context, const void *data, size_t length)
{
    char chunk[64];
...
static void
casdb_sha1_hash_update_internal(struct casdb_sha1_hash_context_t *context, const void *data, size_t length)
{
    size_t i;
    size_t num_chunks;
    size_t leftover_bytes;
    ...
    string = data;
    leftover_bytes = casdb_sha1_handle_leftover(context, data, length);
    hash = context->hash;
...
void
casdb_sha1_hash_update(struct casdb_sha1_hash_context_t *context, struct casdb_stream_t *stream)
{
    do
    {
        casdb_sha1_hash_update_internal(context, stream->buffer, stream->length);
    } while (stream->fill(stream));
}


struct casdb_sha1_hash_t
casdb_sha1_hash_finalize(struct casdb_sha1_hash_context_t *context)
{
    const char *string;
    char last_chunk[128];
    size_t i;
...
struct casdb_sha1_hash_t
casdb_sha1_hash(struct casdb_stream_t *stream)
{
    struct casdb_sha1_hash_context_t context;

    casdb_sha1_hash_init(&context);
    casdb_sha1_hash_update(&context, stream);

    return casdb_sha1_hash_finalize(&context);
}
